...
export function VersionHistoryGraph({
  ...
  useEffect(() => {
    ...
    sortedVersions.forEach((version, index) => {
      ...
      if (isMergeResult && version.notes) {
        const matches = version.notes.match(
          /Merge of versions ([a-f0-9-]+) and ([a-f0-9-]+)/
        )
      ...
      if (index > 0) {
        ...
        // For regular versions, connect to the previous version in the same branch
          ...
          if (previousVersionsInSameBranch.length > 0) {
            ...
            initialNodes[previousVersion.versionId].children.push(
              version.versionId
            )
          } else {
            ...
            if (
              branch &&
              branch.sourceVersionId &&
              initialNodes[branch.sourceVersionId]
            ) {
              currentNode.parents.push(branch.sourceVersionId)
              initialNodes[branch.sourceVersionId].children.push(
                version.versionId
              )
            }
          }
  ...
  const getEdgeColor = (sourceNode: VersionNode, targetNode: VersionNode) => {
    ...
    if (
      targetNode.isMergeResult &&
      targetNode.mergeSourceIds?.includes(sourceNode.id)
    ) {
      return {
          ...
          {branches.map(branch => (
            ...
            >
              ...
              {branch.id === currentBranchId && (
                <Badge
                  variant="outline"
                  className="h-4 text-[10px] bg-primary/10 text-primary"
                >
                  current
                </Badge>
              )}
        ...
        {hoveredNode && (
          ...
          >
            <div className="font-medium mb-1">
              {hoveredNode.isMergeResult ? 'Merge Result' : 'Version'}{' '}
              {getShortId(hoveredNode.id)}
            </div>
            <div className="text-xs text-muted-foreground mb-1">
              {formatDate(hoveredNode.version.timestamp)}
            </div>
            <div className="text-xs text-muted-foreground mb-2">
              Branch: {getBranchName(hoveredNode)}
            </div>

            {/* Show tags for this version */}
            {(() => {
              const versionTags = tags.filter(
                tag => tag.versionId === hoveredNode.id
              )
          ...
          <div className="flex items-start justify-between">
            <div>
              <h4 className="font-medium">
                {selectedNode.isMergeResult ? 'Merge Result' : 'Version'}:{' '}
                {getShortId(selectedNode.id)}
              </h4>
              <p className="text-sm text-muted-foreground">
                {formatDate(selectedNode.version.timestamp)}
              </p>
              <p className="text-sm text-muted-foreground">
                Branch: {getBranchName(selectedNode)}
              </p>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => onVersionSelect(selectedNode.id)}
                      >
                        View Details
                      </Button>
                    </TooltipTrigger>
